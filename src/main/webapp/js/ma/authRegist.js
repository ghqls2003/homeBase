 ...
 * 권한신청 화면에 대한 클래스
 *
 * history : 네이버시스템(주), 1.0, 2018/10/08  초기 작성
 * author : 이광호
 * version : 1.0
...
(function (W, D, $) {
	...
	W.$authRegist = W.$authRegist || {};

	$(document).ready(function() {
		
		$authRegist.ui.pageLoad();		//최초 페이지 로드 시
		$authRegist.event.setUIEvent();
	...
	$authRegist.ui = {
			...
			authRequest : function() {
				...
				if (!$("#agreeChk01").is(":checked")) {
	                alert("(필수)개인정보 수집 및 이용에 동의하셔야 합니다.");
	                $("#agreeChk01").focus();
	                return;
	            }

				if (!$("#agreeChk02").is(":checked")) {
	                alert("(필수)개인정보 제3자 제공에 동의하셔야 합니다.");
	                $("#agreeChk02").focus();
	                return;
	            }
	...
	$authRegist.event = {
			...
			setUIEvent : function() {
				...
				$("#btnRegist").on("click", function() {
					$authRegist.ui.authRequest();
				});

				//사업자번호 확인 버튼
				$("#btnBizrNo").on("click", function() {
					$authRegist.ui.bizrNoConfrim();
				});
