...
(function(W, D, $) {
	...
	W.$delivery = W.$delivery || {};
	var codeType = ''; //  확인결과 조건 , 타입 저장 변수

	$(document).ready(function() {
		$delivery.ui.pageLoad();      //최초 페이지 로드 시
		$delivery.event.setUIEvent();
	});
	...
	$delivery.ui = {
		...
		pageLoad: function() {
			...
			ajax(true, contextPath + '/vfc/deliveryHist/ckResultsList.do', 'body', '처리중입니다.', {}, function(data) {
			...
			$(document).ready(function() {
			    ...
			    $delivery.ui.kendoGridLoad(function() {
		...
		// 확인이력목록
		kendoGridLoad: function(callback) { // 콜백 추가
		    var loadingContainer = $("#grid").closest(".k-grid");
		        ...
		        dataSource: {
		            ...
		            transport: {
		                read: {
		                    ...
		                    url: contextPath + '/vfc/deliveryHist/listView.do',
		                    type: "POST",
		                ...
		                parameterMap: function(options) {
		                    var param = $delivery.event.filter();
		...
		// 엑셀다운로드
		excelDown: function() {
			var fileNm = 'deliveryHist'
			var params = {};
			var data = $delivery.event.filter();
			...
			excelDown("/vfc/deliveryHist/excelBySelectInfo", params, fileNm, totalRowCount);
		}
	...
	$delivery.event = {
		// 조회 버튼 클릭 핸들러
		setUIEvent: function() {
			$(window).on('resize', $delivery.event.resizeGrid);
			$('#btnSearch').on('click', $delivery.event.btnApplyClickHandler);

			// 엑셀다운로드버튼
			$(".excelDownBtn").on("click", function() {
				$delivery.ui.excelDown();
			});
		...
		btnApplyClickHandler: function(e) {
			...
			if (curAuthrtCd == 'Z01') {
				$delivery.event.authrtCdAdmin(start, end);
			} else {
				$delivery.event.authrtCdUser(start, end);
			}
