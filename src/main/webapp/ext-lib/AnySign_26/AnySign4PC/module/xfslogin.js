...
function onload(aDialogObj) {
	...
	if (!AnySign.mXecureFreeSignData.loginPass2) {
		...
		setElementEnable(gInputHandler_passwd2, false);
		__22.disabled = true;
...
function setElementEnable(aElement, aEnable) {
	if(aElement.disabled != true)
	{
		 gOrgBackgroundColor = aElement.style.backgroundColor;
	}
	aElement.disabled = !aEnable;
	if(aEnable)
	{
		if(gOrgBackgroundColor == "")
			gOrgBackgroundColor = '#FFFFFF';
		aElement.style.backgroundColor = gOrgBackgroundColor;
	}
	else
...
function onCheckBoxClick(e) {
	...
	if (target.checked) {
		setElementEnable(gInputHandler_passwd2, true);
		__22.disabled = false;
		__22.style.color = "black";
	} else {
		gInputHandler_passwd2.value = "";
		setElementEnable(gInputHandler_passwd2, false);
		__22.disabled = true;
		__22.style.color = "#bbb";
	}
...
function onRequestCodeButtonClick() {
	...
	__SANDBOX.upInterface().xfsOtpRequest(aUserID, _xfs_request);
}
...
function onOKButtonClick() {
	...
	if (__21.checked) {
		if (gInputHandler_passwd2.value.length == 0) {
	...
	var _xfs_login = function (aResult) {
		if (aResult == 0)
			...
			if (aErrorObject.code == XW_ERROR_USER_LOGIN_INVALID_PASSWD1)
				gInputHandler_passwd.clear();
			else if (aErrorObject.code == XW_ERROR_USER_LOGIN_INVALID_PASSWD2 || aErrorObject.code == XW_ERROR_USER_LOGIN_EXPIRED_PASSWD2)
				gInputHandler_passwd2.value = "";
		}
	}
	...
	var _passwd_inputHandlerCallback = function (result) {
		...
		if (__21.checked) {
			aKeyword2 = gInputHandler_passwd2.value;
...
__24.htmlFor = 'xwup_xfslogin_input3';
...
if (__SANDBOX.IEVersion <= 8) {
__25.mergeAttributes(XWC.UI.createElement("<INPUT name='xfslogin_input3'>"),false);
} else {
__25.setAttribute('name', 'xfslogin_input3', 0);
}
__25.setAttribute('id', 'xwup_xfslogin_input3', 0);
__25.setAttribute('type', 'text', 0);
