...
function readFile(files) {
	var aSignCert, aSignKey, aKmCert, aKmKey;
	...
	var readCert = function(file) {
		...
		reader.onload = function(e) {
			...
			if(count == files.length) {
				if(count == 2) {
					gDialogObj.onconfirm(null, signCert, signKey, null, null);
				} else if(count == 4) {
					gDialogObj.onconfirm(null, signCert, signKey, kmCert, kmKey);
				} else  { 					gDialogObj.onconfirm(pfx, null, null, null, null);
				}
			}
		};
		reader.readAsArrayBuffer(file);
...
__7.setAttribute('accept', '.pfx,.p12,.der,.cer,.key', 0);
__7.setAttribute('multiple', 'multiple', 0);
...
__12.style.margin = '20px 14px 20px 14px';
...
__14.style.display = 'inline-flex';
var __15 = XWC.UI.createElement('IMG');
__15.setAttribute('src', AnySign.mBasePath+'/img/download.png', 0);
