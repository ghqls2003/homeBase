...
function onload(aDialogObj) {
	...
	var aGuideZoneElement = __63;
	XWC.UI.setPFXDragAndDrop ("certmanager", aDragAndDropElement, aGuideZoneElement, function (aPFX, aSignCert, aSignKey, aKmCert, aKmKey, aHandle) {
		var aIsSavePFX = true;
		...
		var inputpasswdDialog = inputpasswdModule({
			...
			onconfirm: function(aResult) { 
				...
				var aSelectedIndex = gTabView.getSelectedIndex ();
				var aSelectedInfo = getSelectedInfoObject (aSelectedIndex, false);

				var _cb_setCertificate = function (aPWCheck) {
					if (aPWCheck == 0 || aPWCheck == 1) { 						loadTableValues();
					} else {
				...
				__SANDBOX.upInterface().setCertificate (aPFX, aSignCert, aSignKey, aKmCert, aKmKey, aResult, aIsSavePFX, aSelectedInfo.searchCondition, "", "", _cb_setCertificate);
			},
			oncancel: function(e) {
				AnySign.mDivInsertOption = pageDivInsertOption;
				inputpasswdDialog.dispose();
	...
	if (AnySign.mOpenCertEnable && gSelectedMediaID == XWC.CERT_LOCATION_LOCALSTORAGE) {
				gSelectedButton = 7;
		createLoader();
...
function loadTabValues (index) {
	...
	if (AnySign.mDefaultCertLocation)
	{
		...
		switch (aMediaType) {
		case XWC.CERT_LOCATION_HARD: 			if (__42.style.display == "block") {
				aCheckedRadioIndex = 0;
				aSelectMediaName = "hard";
			}
			break;
		case XWC.CERT_LOCATION_REMOVABLE: 			if (__45.style.display == "block") {
				aCheckedRadioIndex = 1;
				aSelectMediaName = "removable";
			}
			break;
		case XWC.CERT_LOCATION_ICCARD: 		case XWC.CERT_LOCATION_KEPCOICCARD: 			if (__48.style.display == "block") {
				aCheckedRadioIndex = 2;
				aSelectMediaName = "usbtoken";
			}
			break;
		case XWC.CERT_LOCATION_PKCS11: 			if (__51.style.display == "block") {
				aCheckedRadioIndex = 3;
				aSelectMediaName = "pkcs11";
			}
			break;
		case XWC.CERT_LOCATION_YESSIGNM: 		case XWC.CERT_LOCATION_MPHONE: 			if (__54.style.display == "block") {
				aCheckedRadioIndex = 4;
				aSelectMediaName = "mobile";
			}
			break;
		case XWC.CERT_LOCATION_LOCALSTORAGE: 			if (__39.style.display == "block") {
				aCheckedRadioIndex = 6;
				aSelectMediaName = "localstorage";
			}
			break;
		case XWC.CERT_LOCATION_XECUREFREESIGN: 			if (__60.style.display == "block") {
				aCheckedRadioIndex = 7;
	...
	if (AnySign.mAnySignLiteSupport == true && index == 0) {
		...
		if (AnySign.mDefaultCertLocation && index == 0)
		{
			gMediaRadio.setChecked(aCheckedRadioIndex);
		}
		else
...
function loadTableValues (index, callback) {
		...
		_CB_getCertTreeCallback = function (result)
	{
		...
		case 2:
			...
			if (aSelectedInfo.mediaID == XWC.CERT_LOCATION_LOCALSTORAGE)
				setDragAndDropElement(true, aCertArray);
			else
...
function CertTableView() {
	...
	tableView.onSelectRow = function (aObject) {
		...
		aSelectedCertSource = aSelectedRowElement.getAttribute('source');
	...
	tableView.onRefresh = function (aData) {
		...
		for (i = 0; i < aData.length; i++) {
			...
			if (certInfo[7]) tr.setAttribute('source', certInfo[7]);
	...
	tableView.getSelectedCertSource = function () {
		return aSelectedCertSource || null;
	};
...
function onCopyButtonClick(e) {
	...
	aCertSource = gCertTable.getSelectedCertSource();
	if (!aIssuerRDN || !aCertSerial) {
		alert(XWC.S.noselection);
		return;
	}

	if(gCertTable.getSelectedSubjectRDN().toUpperCase().indexOf("OU=PERSONALB") >= 0 || gCertTable.getSelectedSubjectRDN().toUpperCase().indexOf("OU=CORPORATION4ECB") >= 0)
	{
		aIsBrowserCert = true;
		if (aCertSource == null) {
		...
		_CB_saveCert = function (result, callback)
	{
		...
		else
		{
			...
			aMessage += getMediaName(gSelectedMediaID, aCertSource);
			aMessage += XWC.S.copyok2;
			if (aCertSource == "OPENCERT")
		...
		_CB_certsaveloc = function ()
	{
		if (aSelectInputType == "4pc" && aTargetInputType == "4pc") {
			...
			_exec_syncOpenCert = function () {
				__SANDBOX.upInterface().SyncOpenCert (aCertSource,
													  aCertSerial,
													  aCertKeyword,
													  _CB_syncOpenCert);
			};

			if (AnySign.mOpenCertEnable && aCertSource != null)
			{
				if (aIsBrowserCert) {
										_exec_syncOpenCert ();
				} else {
	...
	certsavelocDialog = certsavelocModule ({
		args: { disableItem: aDisableItem,
				extValues: [aIsBrowserCert, aCertSource]},
		onconfirm: function (result) {
			...
			if (aTargetMediaID == gSelectedMediaID && aCertSource == null) {
								alert('check1');
				_fn_final (_fn_final_callback);
				return;
			}
			...
			if (aCertSource == "BOTH" && aTargetMediaID == XWC.CERT_LOCATION_LOCALSTORAGE)
			{
...
function onExportAction(element) {
	...
	if (aMediaID == XWC.CERT_LOCATION_PKCS11) {
		...
		if(aSubjectRDN.toUpperCase().indexOf("OU=PERSONALB") >= 0 || aSubjectRDN.toUpperCase().indexOf("OU=CORPORATION4ECB") >= 0)
		{
			alert(XWC.S.BrowserCert_notSupported);
			return;
		}
...
function onImportHTML5Action(e) {
		...
		var inputpassDialog = inputpassModule({
			...
			onconfirm: function (aResult) {
				...
				__SANDBOX.upInterface().importCertFromPFX (gSelectedMediaID,
														   aResult,
														   aResult,
														   aPfx, aSignCert, aSignKey, aKmCert, aKmKey,
														   null,
														   0,
	...
	var _pfxdialogOpen = function () {
		...
		var pfxDialog = pfxModule({
			onconfirm: function (aResult, aSignCert, aSignKey, aKmCert, aKmKey) {
				pfxDialog.dispose();
				openPasswdDialog(aResult, aSignCert, aSignKey, aKmCert, aKmKey);
			},
			oncancel: function () {
				alert(XWC.S.importcancel);
				pfxDialog.dispose();
...
function onViewVerifyButtonClick(e) {
	...
	var aCertSource = gCertTable.getSelectedCertSource();
	if (aCertSource == "OPENCERT") {
...
function onChangePassButtonClick(e) {
	...
	var _certtable_reload = function () {
		__SANDBOX.upInterface().getMediaList(XWC.CERT_LOCATION_LOCALSTORAGE, 0, 1, function () {
				loadTableValues (0, function () {
	...
	changepasswdDialog = changepasswdModule({
		...
		onconfirm: function (result, errmsg) {
			switch (result) {
			case 0:
				if (Math.floor(gSelectedMediaID / 100) * 100 == XWC.CERT_LOCATION_LOCALSTORAGE) {
					_certtable_reload();
				} else {
					alert(XWC.S.changeok);
				}
...
function onOwnerVerifyButtonClick(e) {
		...
		_CB_verifyCertOwner = function (result)
	{
		...
		{
			var aErrorObject = __SANDBOX.upInterface().setErrCodeAndMsg();
			if (aErrorObject.code == XW_ERROR_VERIFYCERTOWNER) {
...
function onInstallCertButtonClick(e) {
	...
	try {
		...
		console.log("try catch - window.event, e.target, e.srcElement");
	}
...
function createLoader () {
	...
	loader.id = "xwup_loader_img";
	loader.src = AnySign.mBasePath + "/img/loader.gif";
	loader.className = "xwup-loader-img";

	var table = document.getElementById ('xwup_cert_table');
	if (!table) return;
...
function setDragAndDropElement (aDisplay, aCertList) {
	...
	if (aCertList.length == 0) {
		aGuideZone.style.zIndex = 0;
		aGuideZone.classList.remove ('xwup-drag-guide-certmanager2');
		aGuideZone.childNodes[0].style.visibility = "visible";
		aGuideZone.childNodes[1].style.visibility = "visible";
	} else {
		aGuideZone.style.zIndex = -200;
		aGuideZone.classList.add ('xwup-drag-guide-certmanager2');
		aGuideZone.childNodes[0].style.visibility = "hidden";
		aGuideZone.childNodes[1].style.visibility = "hidden";
	}
}
...
__69.setAttribute('title', XWC.S.table_section, 0);
...
__72.setAttribute('title', XWC.S.table_user, 0);
...
__75.setAttribute('title', XWC.S.table_expire, 0);
...
__78.setAttribute('title', XWC.S.table_issuer, 0);
...
__84.className = 'xwup-group-mng-user';
