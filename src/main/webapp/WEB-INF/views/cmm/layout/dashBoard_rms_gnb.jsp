...
<!-- gnb start -->
<a class="map_menu_btn" style="display: none;">MENU</a>
			...
			<c:forEach items="${sessionScope.menuData}" var="menu">
				<c:if test="${menu.subMenuList != null}">
					...
					<ul class="lnb ${menu.subMenuList[0].upperMenuId}">
						<a href="javascript:void(0)" id="${menu.menuId}" data-gnb="gnb_${menu.menuId}" class="toggle_menu_dept1">
						    <%-- <img src="${contextPath}/images/ico/${menu.menuId}.png" alt="menu"/> --%>
							<span>${menu.menuNm}</span>
						</a>
						<c:forEach items="${menu.subMenuList}" var="subMenu">
							<li><a href="<c:out value='${contextPath}${subMenu.menuUrl}'/>" <c:if test="${requestScope['javax.servlet.forward.request_uri'] == subMenu.menuUrl}"> class="active"</c:if>>${subMenu.menuNm}</a></li>
						</c:forEach>
					</ul>
					<%-- </dd> --%>
				</c:if>
